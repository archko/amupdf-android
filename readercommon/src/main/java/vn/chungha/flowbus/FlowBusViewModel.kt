...
class FlowBusViewModel : ViewModel() {
    ...
    fun <T : Any> collectFlowBus(
        ...
        Log.d(TAG_COLLECT, "collect event : $eventName")
    ...
    fun busEvent(eventName: String, valuePost: Any, delayPost: Long) {
        Log.d(TAG_COLLECT, "post flow bus ==> :$eventName")
        listOfNotNull(
    ...
    private fun <T : Any> invokeReceived(value: Any, onReceived: (T) -> Unit) {
        try {
            ...
            Log.d(TAG_COLLECT, "class cast error when received value ==> :$value")
        } catch (e: Exception) {
            Log.d(TAG_COLLECT, "exception error when received value ==> :$value")
        }
    }
